...
class ScriptedEvalLLM:
    ...
    def generate_structured_with_metadata(self, system_prompt: str, user_prompt: str) -> StructuredLLMResult:
        ...
        if "classify user intent" in system_prompt:
            data = self.case["classifier_result"]
        elif "planner for a safe fitness coaching agent" in system_prompt:
            data = self.case["planner_result"]
        else:
            data = {}
        return StructuredLLMResult(
            ok=True,
...
class RemediationPhase6EvalTests(unittest.IsolatedAsyncioTestCase):
    def test_golden_fixture_has_ci_gate_metadata(self) -> None:
        ...
        self.assertGreaterEqual(len(cases), fixture["thresholds"]["minimum_cases"])
        self.assertEqual(fixture["thresholds"]["minimum_intent_accuracy"], 1)
        for case in cases:
            with self.subTest(case=case["id"]):
                self.assertIn("request", case)
                self.assertIn("classifier_result", case)
                self.assertIn("planner_result", case)
                self.assertIn("expected", case)
                self.assertTrue(case["expected"]["intent"])
